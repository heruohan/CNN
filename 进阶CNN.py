# -*- coding: utf-8 -*-
"""
Created on Mon Jun 25 02:02:58 2018
...
###定义训练轮数max_steps,bacth_size等
max_steps=3000
batch_size=128
data_dir=r'E:\tmp\cifar10_data\cifar-10-batches-bin'


####初始化weights,进行权重的L2正则化.
####使用截断的正态分布初始化权重.
...
####使用cifar10_inpuut类中的distorted_inputs函数产生训练
##需要的数据，包括特征及其对应的label.
images_train,labels_train=cifar10_input.distorted_inputs(\
                data_dir=data_dir,batch_size=batch_size)



####使用cifar10_input.input函数生成测试数据.
...
weight1=variable_with_weight_loss(shape=[5,5,3,64],stddev=5e-2,\
                                  w1=0.0)
kernel1=tf.nn.conv2d(image_holder,weight1,[1,1,1,1],padding=\
                'SAME')
...
######构建最后一层.
weight5=variable_with_weight_loss(shape=[192,10],stddev=\
                                  1/192.0,w1=0.0)
...
def loss(logits,labels):
    ...
    return(tf.add_n(tf.get_collection('losses'),name=\
                    'total_loss'))
    
#########将logits节点和label_holder传入loss函数获得最终
###的loss.
...
#######使用tf.InteractiveSession创建默认的session,接着初始化
###全部模型参数.
...
for step in range(max_steps):
    ...
    if(step%10==0):
        examples_per_sec=batch_size/duration #每秒钟训练的样本数
        sec_per_batch=float(duration)
        
        format_str=('step %d,loss=%.2f (%.1f examples/sec; \
                                %.3f sec/batch)')
