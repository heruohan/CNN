# -*- coding: utf-8 -*-
"""
Created on Sun Jul  8 21:52:33 2018
...
###实现conv_op函数,用来创建卷积层并把本层参数存入参数列表.
def conv_op(input_op,name,kh,kw,n_out,dh,dw,p): #p参数列表
...
def run_benchmark():
    with tf.Graph().as_default():
        ...
        time_tensorflow_run(sess,predictions,{keep_prob:1.0},\
                            'Forward')
        objective=tf.nn.l2_loss(fc8)
        grad=tf.gradients(objective,p)
        time_tensorflow_run(sess,grad,{keep_prob:0.5},\
                            'Forward-backward')
